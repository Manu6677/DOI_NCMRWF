...
const ProductCard = ({ title, desc, img, link }) => {
  return (
    <div className="group relative flex h-[27rem] max-w-lg items-center justify-center overflow-hidden rounded-lg bg-white transition-all duration-300 hover:scale-105">
      ...
      <div className="absolute inset-0 m-0 size-full overflow-hidden rounded-none bg-blue-700 bg-cover bg-center">
        <div className="absolute inset-0 size-full bg-gradient-to-t from-black/80 via-black/50"></div>
      </div>
      <div className="relative px-6 py-10 text-center md:px-12">
        ...
        <img
          alt={`${title}-image`}
          src={img}
          className="relative inline-block size-20"
        />
      </div>
