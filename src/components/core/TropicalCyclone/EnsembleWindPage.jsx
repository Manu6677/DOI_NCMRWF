...
const EnsembleWindPage = () => {
  ...
  useEffect(() => {
    ...
    const baseWindUrl = `${ASSETS_BASE_URL}/${BASE_PATH}/${formattedDate}/12-km-ENSEMBLE-Outputs/Ensemble-Stamps/neps_stamp-wind-day`;
    const urls = [];
    for (let i = 0; i <= 9; i++) {
      urls.push(`${baseWindUrl}${i + 1}.png?v=${Date.now()}`);
    }
    setImageUrls(urls);
    console.log('setImageUrls wind', urls);
  }, []);
...
export default EnsembleWindPage;
